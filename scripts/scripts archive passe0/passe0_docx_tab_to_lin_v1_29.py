...
def transformer_tableau_et_images(doc_path):
    ...
    for table in doc.tables:
        for row in table.rows:
            ...
            sous_lignes = ligne.split("\n")
            sous_lignes = [s.strip() for s in sous_lignes if s.strip() != '']
            for sous_ligne in sous_lignes:
                if sous_ligne.startswith('Défi |') or sous_ligne.startswith('Herausforderung |'):
                    if not entete_inseree:
                        langue = 'fr' if sous_ligne.startswith('Défi |') else 'de'
                        lignes_out.extend(generer_entete(langue, id_code, nom_fichier))
                        entete_inseree = True
                    continue
                # Découpage spécial pour #Q / #A avec séparateur
                if sous_ligne.strip().startswith('#Q'):
                    lignes_out.append('#Q' + '#' * 38)
                    lignes_out.append(sous_ligne[len('#Q#######'):].strip())
                elif sous_ligne.strip().startswith('#A'):
                    lignes_out.append('#A' + '-' * 39)
                    lignes_out.append(sous_ligne[len('#A-------'):].strip())
                else:
                    lignes_out.extend(decouper_balises_speciales(sous_ligne))
            if not ligne.strip():
    ...
    # Ajout de paragraphes dans Word avec séparateurs visuels
    for l in lignes_out:
        p = doc_out.add_paragraph(l)
        p.paragraph_format.space_after = 0
        if l.strip().startswith('#Q'):
            p = doc_out.add_paragraph('#' * 40)
            p.paragraph_format.space_after = 0
        elif l.strip().startswith('#A'):
            p = doc_out.add_paragraph('-' * 40)
            p.paragraph_format.space_after = 0
    part = doc.part
