...
def extraire_images_des_cellules(doc, image_dir, prefix, lignes_out_ref):
    images = []
    images_par_cellule = {}
    hash_dict = {}
    index = 1
...
#                                    print(f"[DEBUG] Image extraite : {image_name} (table {cle[0]}, ligne {cle[1]}, cellule {cle[2]})")
                                ...
                                lignes_out_ref.append(f"dict:  {cle} : ['{image_name}']")
...
#    print("=== TEST POINT 1===")
    ...
    images_par_cellule = extraire_images_des_cellules(doc, IMAGE_DIR, prefix, lignes_out)
...
#                    print(f"[TRACE] dict inséré dans lignes_out : {cle}")
    ...
    while ligne_idx < len(lignes_out):
        ...
        if "#Q" in ligne and "|" in ligne:
            ...
            lignes_reformatees.extend(["", "", "#Q########################################################", "", "", "", q_texte, "", "#A-----------------------------------------------------------------------------------", "", "", ""])
            ...
            # Si des lignes dict: suivent immédiatement, les recopier
            lookahead_idx = ligne_idx + 1
            while lookahead_idx < len(lignes_out) and lignes_out[lookahead_idx].startswith("dict:"):
                lignes_reformatees.append(lignes_out[lookahead_idx])
                lookahead_idx += 1

            ligne_idx = lookahead_idx  # on saute les lignes dict
        else:
            lignes_reformatees.append(ligne)
            ligne_idx += 1
