...
def transformer_tableau_et_images(doc_path):
    ...
    for table in doc.tables:
        for row in table.rows:
            # Éliminer les lignes signalant la fin de la langue
            if any(phrase in ligne for phrase in [
                'Temps passé à ce défi',
                'Für diese Herausforderung aufgewendete Zeit'
            ]):
            ...
            # Vérifier si cette ligne marque la fin d'une langue
            if any(phrase in ligne for phrase in [
                'Temps passé à ce défi',
                'Für diese Herausforderung aufgewendete Zeit'
            ]):
                if entete_inseree:
                    ...
                    lignes_out.extend(generer_entete(langue, id_code, nom_fichier))
                continue
            # Vérifier si cette ligne marque la fin des blocs
            if any(phrase in ligne for phrase in [
                'Temps passé à ce défi',
                'Für diese Herausforderung aufgewendete Zeit'
            ]):
    ...
    doc_out = Document()
    # Ajout d’un paragraphe Word pour chaque sous-ligne (↵ devient fin de paragraphe)
    # Réduction à un seul saut de ligne vide max
    cleaned_out = []
    empty = False
    # Nettoyage des paragraphes : pas plus d'un vide à la suite
