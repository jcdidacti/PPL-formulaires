# ==============================================================================
# Script : passe0_docx_tab_to_lin_v1_22.py
...
def transformer_tableau_et_images(doc_path):
    ...
    doc_out = Document()
    for ligne in lignes:
        if ligne.strip().startswith('Défi |') or ligne.strip().startswith('Herausforderung |'):
        ...
        for ligne in lignes:
            ...
            if not entete_inseree:
                if ligne_strip.startswith('Défi |'):
                    langue = 'fr'
                    lignes_out.extend(generer_entete(langue, id_code, nom_fichier))
                    entete_inseree = True
                elif ligne_strip.startswith('Herausforderung |'):
                    langue = 'de'
                    lignes_out.extend(generer_entete(langue, id_code, nom_fichier))
                    entete_inseree = True
    ...
    with log_path.open("w", encoding="utf-8") as f:
        f.write(f"# Script : passe0_docx_tab_to_lin_v1_22.py\n")
...
with log_global_path.open("w", encoding="utf-8") as f_global:
    f_global.write(f"# Script : passe0_docx_tab_to_lin_v1_22.py\n")
