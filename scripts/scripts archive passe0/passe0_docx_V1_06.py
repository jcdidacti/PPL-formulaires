...
SCRIPT_VERSION = "v1_06"
print(f"--- Lancement {SCRIPT_VERSION} ---")
...
def transformer_tableau_et_images(doc_path):
    ...
    for l in lignes_out:
        ...
        for sous_ligne in l.splitlines():
            if '#IMGQ#' in sous_ligne or '#IMGA#' in sous_ligne:
                sous_ligne_clean = sous_ligne.replace('#IMGQ#','').replace('#IMGA#','').strip()
                if sous_ligne_clean:
                    p = doc_out.add_paragraph(sous_ligne_clean)
                    p.paragraph_format.space_after = 0
                    p.paragraph_format.space_before = 0
                    p.paragraph_format.line_spacing = 1
                if image_index < len(image_paths) and image_paths[image_index]:
                    try:
                        print(f'Insertion image : {image_paths[image_index].name}')
                        doc_out.add_picture(str(image_paths[image_index]), width=Inches(4.5))
                        image_index += 1
                    except Exception as e:
                        doc_out.add_paragraph(f'[Erreur insertion image: {e}]')
                else:
                    print('Aucune image disponible à insérer pour cette balise')
                    try:
                        print(f'Insertion image : {image_paths[image_index].name}')
                        doc_out.add_picture(str(image_paths[image_index]), width=Inches(4.5))
                        image_index += 1
    ...
    # Insertion des images à l'endroit des balises
    image_paths = []
