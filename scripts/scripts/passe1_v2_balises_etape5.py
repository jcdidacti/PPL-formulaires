...
# ============================================================================
# Script : passe1_v2_balises_etape5.py
# Objectif : Étape 5 — Ajout des balises de structure après #Author
# ============================================================================
import re
import shutil
...
def formater_bloc_langue(bloc, base_name, langue, date_now):
    ...
    en_tete = [
        "##Identification",
        f"#Script : passe1_v2_balises_etape5.py",
        f"#Run at : {date_now}",
        f"#ID file : {base_name}",
        f"##LANG-{langue}",
        f"#ID        {id_code}",
    ...
    for line in bloc:
        contenu.append(line)
        if not intro_added and line.strip().lower().startswith("#author"):
            contenu.append("")
            contenu.append("##Introduction")
            contenu.append("[texte d’introduction]")
            contenu.append("##Work Start")
            intro_added = True
